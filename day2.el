;;; day2.el --- Day 2 Advent of Code 2020            -*- lexical-binding: t; -*-
...
(defun day2-part1-solution (&optional password-validation-func)
  "Answer: 414.

Specify PASSWORD-VALIDATION-FUNC for particular solution."
  (interactive)
  (let ((lines (split-string day2-input "\n"))
        (nvalid-passwords 0)
        (password-validation-func (or password-validation-func
                                      'day2--password-valid-p)))
    (cl-loop for line in lines
             do (let* ((tokens (split-string line))
                       (tokens-alist `((rules . ,(mapcar
                                                  'cl-parse-integer
                                                  (split-string (pop tokens) "-")))
                                       (char . ,(string-to-char (pop tokens)))
                                       (password . ,(pop tokens)))))
                  ;; (when (day2--password-valid-p tokens-alist)
                  (when (funcall password-validation-func tokens-alist)
                    (setq nvalid-passwords (1+ nvalid-passwords)))))
    nvalid-passwords))


(defun day2--password-valid-p (tokens-alist)
  "Check if TOKENS-ALIST is a valid password."
  (let ((ntimes (day2--char-in-string-ntimes (cdr (assoc 'char tokens-alist))
                                             (cdr (assoc 'password tokens-alist))))
        (rule-min-ntimes (nth 0 (cdr (assoc 'rules tokens-alist))))
        (rule-max-ntimes (nth 1 (cdr (assoc 'rules tokens-alist)))))
    (and (>= ntimes rule-min-ntimes)
         (<= ntimes rule-max-ntimes))))


(defun day2--char-in-string-ntimes (char string)
  "Count number of times CHAR occurs in STRING."
  (cl-loop for c across string
           count (equal c char)))


(defun day2-part2-solution ()
  "Answer: ."
  (interactive)
  (day2-part1-solution 'day2--password-valid-part2-p))


(defun day2--password-valid-part2-p (tokens-alist)
  "Check if TOKENS-ALIST is a valid password."
  (let ((char (cdr (assoc 'char tokens-alist)))
        (password (cdr (assoc 'password tokens-alist)))
        (rule0-index (- (nth 0 (cdr (assoc 'rules tokens-alist))) 1))
        (rule1-index (- (nth 1 (cdr (assoc 'rules tokens-alist))) 1)))
    (xor (equal char (aref password rule0-index))
         (equal char (aref password rule1-index)))))
