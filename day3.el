...
(defun day3-part1-solution (&optional password-validation-func)
  "Answer: 414.

Specify PASSWORD-VALIDATION-FUNC for particular solution."
  (interactive)
  (let ((lines (split-string day3-input "\n"))
        (nvalid-passwords 0)
        (password-validation-func (or password-validation-func
                                      'day3--password-valid-p)))
    (cl-loop for line in lines
             do (let* ((tokens (split-string line))
                       (tokens-alist `((rules . ,(mapcar
                                       ...
                                       (password . ,(pop tokens)))))
                  ;; (when (day3--password-valid-p tokens-alist)
                  (when (funcall password-validation-func tokens-alist)
                    (setq nvalid-passwords (1+ nvalid-passwords)))))
    nvalid-passwords))


(defun day3--password-valid-p (tokens-alist)
  "Check if TOKENS-ALIST is a valid password."
  (let ((ntimes (day3--char-in-string-ntimes (cdr (assoc 'char tokens-alist))
...
(defun day3--char-in-string-ntimes (char string)
  "Count number of times CHAR occurs in STRING."
  (cl-loop for c across string
           count (equal c char)))


(defun day3-part2-solution ()
  "Answer: ."
  (interactive)
  (day3-part1-solution 'day3--password-valid-part2-p))


(defun day3--password-valid-part2-p (tokens-alist)
  "Check if TOKENS-ALIST is a valid password."
